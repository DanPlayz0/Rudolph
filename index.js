(async () => {
  ...
  if (Number(process.version.split('.')[0].slice(1)) < 16) return console.error('Node version 16 or higher is required!');

  if (!config.token) return console.error('Missing token provided');
  ...
  if ((gatewaySettings?.shards||1) > 1) totalShards = Math.ceil((gatewaySettings.shards * (1_000 / guildsPerShard)) / multipleOf) * multipleOf;
