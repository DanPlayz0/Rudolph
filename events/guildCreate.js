...
module.exports = class extends Event {
  ...
  async run(client, guild) {
    ...
    if (!db) await client.database.insertOne('guilds', { id: guild.id, botInServer: true, discord: { name: guild.name, icon: guild.icon } });
    else await client.database.updateOne('guilds', {id: guild.id}, {$set:{botInServer: true, discord: { name: guild.name, icon: guild.icon } }});
  }
};
